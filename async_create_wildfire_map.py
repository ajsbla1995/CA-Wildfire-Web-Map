...
async def fetch_calfire_geojson():
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url, headers=headers) as response:
            if response.status == 200:
                geojson_data = await response.json()  # Asynchronously parse the JSON
                return geojson_data
            else:
                print(f"Failed to fetch GeoJSON data. Status code: {response.status}")
                return None



# Function to process GeoJSON data into a DataFrame
def process_geojson_to_df(geojson_data):
    ...
    # Optionally, you can drop the 'properties' prefix in the column names
...
async def fetch_wildfire_perims_nifc():
    ''' 
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url_geojson, headers=headers) as response:
            if response.status == 200:
                geojson_data = await response.json()  # Asynchronously parse the JSON
                return geojson_data
            else:
...
async def fetch_red_flag_data(session, endpoint, parameters, headers):
    async with session.get(endpoint, params=parameters, headers=headers) as response:
        if response.status == 200:
            return await response.json()
        else:
            print(f"Failed to retrieve Red Flag Warning data. Status code: {response.status}")
            return None


async def add_red_flag_warning(state, fire_weather_zones, m):
    '''
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for event in events:
            ...
            tasks.append(fetch_red_flag_data(session, endpoint, parameters, headers))
    ...
    for _, row in zones_df.iterrows():
        ...
        folium.GeoJson(
            ...
            style_function=lambda feature: {
                'fillColor': '#fe9929',  # Fill color of the polygon
                'color': '#fe9929',  # Outline color of the polygon
...
async def fetch_excessive_heat_data(session, endpoint, parameters, headers):
    async with session.get(endpoint, params=parameters, headers=headers) as response:
        if response.status == 200:
            return await response.json()
        else:
            print(f"Failed to retrieve Excessive Heat Warning data. Status code: {response.status}")
            return None

async def add_excessive_heat_warning(state, nws_zones, m):
    '''
    GET request to National Weather Service API for Special NWS warnings 'Excessive Heat Warning' and 'Excessive Heat Watch'.
    ...
    Returns
    -------
    Map object with added Excessive Heat Warning layer. 
    '''
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for event in events:
            parameters = {'event': event}
            tasks.append(fetch_excessive_heat_data(session, endpoint, parameters, headers))
...
def deprecated_get_current_weather_conditions(lat, lon):
...
async def get_current_weather_conditions(lat, lon):
    ...
    ['periods'][0] indicates the current day, ['periods'][0] is the forecast for night, etc.
    We are querying temperature, relative humidity, wind speed, and wind direction.
    '''
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(endpoint, headers=headers) as response:
            if response.status != 200:
                return f"Error: Unable to retrieve forecast. Status code {response.status}, Response: {await response.text()}"

            data = await response.json()
            forecast_url = data['properties']['forecast']
            gridpoint_url = data['properties']['forecastGridData']
        
        # Get the current local weather from the forecast URL
        async with session.get(forecast_url, headers=headers) as response:
            if response.status != 200:
                return f"Current weather data for location is unavailable ({response.status})"
        ...
        # Get the relative humidity from the gridpoint URL
        async with session.get(gridpoint_url, headers=headers) as response:
            if response.status != 200:
                return f"Unable to retrieve gridpoint data for location ({response.status})"
            
            gridpoint_data = await response.json()
            try:
                humidity = gridpoint_data['properties']['relativeHumidity']['values'][0]['value']
            except KeyError as e:
                humidity = 'currently not able to be retrieved.'
